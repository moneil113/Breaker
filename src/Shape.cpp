...
void Shape::setOffsets(std::shared_ptr<std::vector<float> >) {
    glGenBuffers(1, &offBufID);
    glBindBuffer(GL_ARRAY_BUFFER, offBufID);
    glBufferData(GL_ARRAY_BUFFER, offBuf.size()*sizeof(float), &offBuf[0], GL_STATIC_DRAW);
}
...
void Shape::drawInstanced(const std::shared_ptr<Program> prog, int count) const {
    // Bind position buffer
    int h_pos = prog->getAttribute("aPos");
    glEnableVertexAttribArray(h_pos);
    glBindBuffer(GL_ARRAY_BUFFER, posBufID);
    ...
    // Bind offset buffer
    int h_off = prog->getAttribute("offset");
    if (h_off != -1 && offBufID != 0) {
        glEnableVertexAttribArray(h_off);
        glBindBuffer(GL_ARRAY_BUFFER, offBufID);
        glVertexAttribPointer(h_off, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (const void *)0);
        glVertexAttribDivisor(h_off, 1);
    }
    ...
    // Draw
    glDrawElementsInstanced(GL_TRIANGLES, (int)eleBuf.size(), GL_UNSIGNED_INT, (const void *)0, count);
